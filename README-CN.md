...
- **静止指令默认解除嫌疑**——收到停车指令后，机器人不再主动制造画面变化（工程先验）。注意这是先验而非事实：刹车惯性、车体振动、云台回正仍可能带来残余运动，严格场景应叠加里程计/IMU 零速校验（路线图：多源 EgoStateProvider）。
...
`render_path(points)` 预留真实规划器路径投影（世界→图像标定由上层完成）。
